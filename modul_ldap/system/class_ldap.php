...
class class_ldap {
    ...
    private function connect() {
        if($this->objCx == null) {
            ...
            class_logger::getInstance()->addLogRow("new ldap-connection to ".$this->objConfig->getConfig("ldap_server").":".$this->objConfig->getConfig("ldap_port"), class_logger::$levelInfo);
    ...
    public function getMembersOfGroup($strGroupDistinguishedName) {
        ...
        if($objResult !== false) {
            class_logger::getInstance()->addLogRow("ldap-search found ".ldap_count_entries($this->objCx, $objResult)." entries", class_logger::$levelInfo);
