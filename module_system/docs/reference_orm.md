...
Nevertheless, in many cases you may want to add restrictions to the list-query (WHERE restrictions when talking about sql). Therefore you may add OrmCondition instances to the list-query:

	$objOrmList = new OrmObjectlist();
	$objOrmList->addWhereRestriction(new OrmCondition("AND custnr = ?", array($intNumber)));
...
This restriction extends the normal IN-Condition by searching also for empty cells (NULL or '').
