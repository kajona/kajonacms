	...
	private function pagesFolderBrowser($strFolder, $bitPages, $strElement, $strPageid = "0" ) {
		...
		if(!$bitPages || $strLevelUp != "") {
			...
			if($strFolder == 0 && !$bitPages)
				$strAction .= $this->objToolkit->listButton("<a href=\"#\" title=\"".$this->getText("ordner_uebernehmen")."\" onmouseover=\"kajonaAdminTooltip.add(this);\" onclick=\"window.opener.kajonaUtils.folderviewSelectCallback([['ordnerid', '0'], ['".$strElement."', '']]); self.close();\">".getImageAdmin("icon_accept.gif"));
		...
		if(count($arrFolder) > 0 && $strPageid == "0") {
			foreach($arrFolder as $objSingleFolder) {
				if($bitPages) {
					...
					$strAction .= $this->objToolkit->listButton("<a href=\"#\" title=\"".$this->getText("ordner_uebernehmen")."\" onmouseover=\"kajonaAdminTooltip.add(this);\" onclick=\"window.opener.kajonaUtils.folderviewSelectCallback([['ordnerid', '".$objSingleFolder->getSystemid()."'], ['".$strElement."', '".$objSingleFolder->getStrName()."']]); self.close(); \">".getImageAdmin("icon_accept.gif"));
					$strReturn .= $this->objToolkit->listRow2($objSingleFolder->getStrName(), $strAction, $intCounter++);
				}
			}
		...
		if($bitPages && $strPageid == "0") {
			...
			if(count($arrPages) > 0) {
				...
				foreach($arrPages as $objSinglePage) {
					...
					$strAction .= $this->objToolkit->listButton("<a href=\"#\" title=\"".$this->getText("seite_uebernehmen")."\" onmouseover=\"kajonaAdminTooltip.add(this);\" onclick=\"window.opener.kajonaUtils.folderviewSelectCallback([['".$strElement."', '".$arrSinglePage["name2"]."']]); self.close();\">".getImageAdmin("icon_accept.gif")."</a>");
