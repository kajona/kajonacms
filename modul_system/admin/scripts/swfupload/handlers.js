...
function swfUploadLoaded() {
	...
	document.getElementById("btnBrowse").onclick = function () { if (self.getSetting("file_queue_limit") == 1) {self.selectFile();} else {self.selectFiles();} };
	document.getElementById("btnCancel").onclick = function () { self.cancelQueue(); this.style.display = "none";};
}
...
function fileDialogComplete(numFilesSelected, numFilesQueued) {
	try {
		if (numFilesSelected > 0) {
			...
			document.getElementById("fsUploadProgress").style.display = "";
		}
...
function queueComplete(numFilesUploaded) {
	...
	// If there's no error occurred during file uploads, refresh page
	if (this.uploadErrorOccurred == false) {
		location.reload();
	}
}