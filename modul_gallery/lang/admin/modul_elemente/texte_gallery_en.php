<?php
/*"******************************************************************************************************
*   (c) 2004-2006 by MulchProductions, www.mulchprod.de                                                 *
*   (c) 2007-2009 by Kajona, www.kajona.de                                                              *
*       Published under the GNU LGPL v2.1, see /system/licence_lgpl.txt                                 *
*-------------------------------------------------------------------------------------------------------*
*   $Id$                                         *
********************************************************************************************************/

// --- Module texts -------------------------------------------------------------------------------------

$lang["gallery_id"]                               = "Gallery:";
$lang["gallery_template"]                         = "Template:";
$lang["gallery_maxh_p"]                           = "Max height:";
$lang["gallery_maxh_d"]                           = "Max width:";
$lang["gallery_maxw_m"]                           = "Max width:";
$lang["gallery_maxw_p"]                           = "Max height:";
$lang["gallery_maxw_d"]                           = "Max width:";
$lang["gallery_maxh_m"]                           = "Max height:";
$lang["gallery_nrow"]                             = "Number of images per row:";
$lang["gallery_text"]                             = "Inscription:";
$lang["gallery_text_x"]                           = "X-axis:";
$lang["gallery_text_y"]                           = "Y-axis:";
$lang["hint_preview"]                             = "Preview<br />If the picture exceeds one of the values, it will be scaled (0 = no scaling)";
$lang["hint_detail"]                              = "Detailview<br />If the picture exceeds one of the values, it will be scaled (0 = no scaling)";
$lang["hint_movie"]                               = "Moviestrip view<br />If the picture exceeds one of the values, it will be scaled (0 = no scaling)";
$lang["hint_text"]                                = "Places a text at the position (X,Y) into the image when in detail-mode";
$lang["hint_nrow"]                                = "Number of images per row. 0 = unlimited, templatesection piclist_unlimited is used, > 0 = limited, templatesection piclist is used.";
$lang["gallery_mode"]                             = "Element-mode";
$lang["gallery_mode_hint"]                        = "Sets up the mode of the element: Regular list-/detailmode, or a random picture out of the current gallery. The values defined in the detail-view section are applied.";
$lang["gallery_imagesperpage"]                    = "Images per page:";
$lang["gallery_imagesperpage_hint"]               = "Number of images per page in the listview. 0 = unlimited.";
$lang["mode_standard"]                            = "Standard-mode";
$lang["mode_random"]                              = "Random-mode";

?>