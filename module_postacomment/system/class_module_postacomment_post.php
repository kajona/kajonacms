...
class class_module_postacomment_post extends class_model implements interface_model, interface_sortable_rating, interface_admin_listable, interface_search_portalobject {
    ...
    public function updateSearchResult(class_search_result $objResult) {
        $objResult->setStrPagelink(class_link::getLinkPortal($this->getStrAssignedPage(), "", "_self", $this->getStrAssignedPage(), "", "&highlight=".urlencode(html_entity_decode($objResult->getStrQuery(), ENT_QUOTES, "UTF-8"))));
        $objResult->setStrPagename($this->getStrAssignedPage());
