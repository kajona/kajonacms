		...
		elseif ($strMode == "edit") {
			...
			if($this->objRights->rightEdit($this->getSystemid())) {
                ...
                if($objLanguageManager != null) {
                    ...
                    $arrToolbarEntries[2] .= $this->objToolkit->formInputDropdown("news_languageswitch", $arrDD, "", $this->getSystemid(), "inputDropdown", true, "onchange=\"window.location='".getLinkAdminHref("news", "editNews").(_system_mod_rewrite_ == "true" ? "?" : "&")."systemid='+this.value;\"");
	...
	public function actionEditNewscontent() {
		...
		if($this->objRights->rightRight1($this->getSystemid())) {
            ...
            if($objLanguageManager != null) {
                ...
                $arrToolbarEntries[2] .= $this->objToolkit->formInputDropdown("news_languageswitch", $arrDD, "", $this->getSystemid(), "inputDropdown", true, "onchange=\"window.location='".getLinkAdminHref("news", "editNewscontent").(_system_mod_rewrite_ == "true" ? "?" : "&")."systemid='+this.value;\"");
                $arrToolbarEntries[2] .= "</div>";
            }
