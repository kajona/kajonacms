			...
			if(count($arrNews) > 0) {
				foreach($arrNews as $arrOneNews) {
					...
					if(isset($this->arrHits[$arrOneNews["system_id"]]["hits"]))
    					...
    					$this->arrHits[$arrOneNews["system_id"]]["pagelink"] = getLinkPortal(_news_search_resultpage_, "", "_self", $arrOneNews["news_title"], "newsDetail", "&highlight=".$this->strSearchtermRaw, $arrOneNews["system_id"]);
